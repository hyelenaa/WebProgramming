...
var Modal = React.createClass({
  ...
  render: function render() {
    ...
    return React.createElement(
      'div',
      { id: 'myModal', className: 'modal', style: { display: "block" } },
      React.createElement(
        'div',
...
var LoginForm = React.createClass({
  ...
  render: function render() {
    return React.createElement(
      'div',
      { className: 'login-background-box' },
      React.createElement(
        'div',
        { className: 'login-form' },
        React.createElement('img', { src: '/images/Fire.png', className: 'fire-image', alt: 'Fire Image' }),
        React.createElement(
          'h2',
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'button',
              { type: 'submit', className: 'login-button login-button-text' },
              '로그인'
            ),
            React.createElement(
              'button',
              { type: 'button', onClick: this.props.onSignupClick, className: 'signup-button signup-button-text' },
              '회원가입'
            )
          ),
          React.createElement(Modal, { show: this.state.showModal, message: this.state.errorMessage, onClose: this.closeModal })
        )
      )
    );
  }
...
var SignupForm = React.createClass({
  ...
  handleSubmit: function handleSubmit(e) {
    ...
    getUser(username).then((function (user) {
      ...
      if (user) {
        ...
        this.setState({ errorMessage: '회원가입에 성공했습니다!', showModal: true }, function () {
          setTimeout(function () {
  ...
  render: function render() {
    return React.createElement(
      'div',
      { className: 'signup-background-box' },
      React.createElement(
        'div',
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'button',
              { type: 'button', onClick: this.props.onBack, className: 'previous-button previous-button-text' },
              '뒤로'
            ),
            React.createElement(
              'button',
              { type: 'submit', className: 'signup-button signup-button-text' },
              '회원가입'
            )
          ),
          React.createElement(Modal, { show: this.state.showModal, message: this.state.errorMessage, onClose: this.closeModal })
...
var RoomSearchForm = React.createClass({
  displayName: 'RoomSearchForm',

  getInitialState: function getInitialState() {
    return { roomName: '' };
  },

  handleRoomNameChange: function handleRoomNameChange(e) {
  ...
  render: function render() {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'button',
          { type: 'submit', className: 'next-button button-text' },
          '검색'
        )
      )
    );
...
var RoomList = React.createClass({
  ...
  createRoom: function createRoom() {
    ...
    getRoom(this.props.searchedRoom, function (err, room) {
      if (err) {
      ...
      if (room) {
        alert('이미 존재하는 방입니다.');
        _this2.props.onRoomCreated(_this2.props.searchedRoom);
      } else {
        addRoom(_this2.props.searchedRoom, function (err) {
          if (err) {
          ...
          _this2.props.onRoomSearch(_this2.props.searchedRoom);
        });
      }
    });
  },
  render: function render() {
    ...
    var searchedRoom = _props.searchedRoom;
    var roomExists = _props.roomExists;
    var onCancel = _props.onCancel;
    var onRoomSelect = _props.onRoomSelect;

    return React.createElement(
      'div',
      { className: 'room-list' },
      searchedRoom && !roomExists && React.createElement(
        'div',
        { className: 'create-room-container' },
        React.createElement(
          'h2',
          { className: 'create-room-message' },
          '채팅방이 존재하지 않습니다. 새로 만드시겠습니까?'
        ),
        React.createElement(
          'div',
          { className: 'create-room-buttons' },
          React.createElement(
            'button',
            { onClick: this.createRoom, className: 'create-room-button' },
            '예'
          ),
          React.createElement(
            'button',
            { onClick: onCancel, className: 'cancel-room-button' },
            '아니오'
          )
        )
      ),
      searchedRoom && roomExists && React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            'li',
            { onClick: function () {
                return onRoomSelect(searchedRoom);
              } },
            searchedRoom
          )
        )
      ),
      !searchedRoom && React.createElement(
        'div',
        null,
        React.createElement(
          'h3',
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        this.props.messages.map(function (message, i) {
          return React.createElement(
            'li',
            { key: i, className: 'message-item ' + (message.user === _this3.props.user ? 'self' : '') },
            React.createElement(
              'strong',
              null,
              message.user
...
var ChatApp = React.createClass({
  ...
  _initialize: function _initialize(data) {
    ...
    this.setState({ users: users, username: name, rooms: rooms });
  },
  ...
  _roomExists: function _roomExists(data) {
    this.setState({ roomExists: data.exists });
  },

  _loadMessages: function _loadMessages(messages) {
    this.setState({ messages: messages });
  },
  ...
  handleMessageSubmit: function handleMessageSubmit(message) {
    ...
    addMessageToRoom(currentRoom, message, function (err) {
      if (err) {
  ...
  handleChangeName: function handleChangeName(newName) {
    ...
    socket.emit('change:name', { name: newName }, function (result) {
      ...
      _this4.setState({ users: users, username: newName });
    });
  },

  handleLogin: function handleLogin(username) {
    this.setState({ username: username, loggedIn: true }, function () {
  ...
  handleSignupBack: function handleSignupBack() {
    this.setState({ signup: false });
  },

  handleRoomSearch: function handleRoomSearch(roomName) {
    var _this5 = this;
    ...
    getRoom(roomName, function (err, room) {
      ...
      _this5.setState({ roomSearched: true, searchedRoom: roomName, roomExists: !!room });
    });
  },

  handleRoomCreated: function handleRoomCreated(roomName) {
    var _this6 = this;

    addRoom(roomName, function (err) {
      if (err) {
        console.error("Failed to create room: ", err);
        return;
      }

      _this6.handleRoomSearch(roomName);
    });
  },

  handleRoomSelect: function handleRoomSelect(roomName) {
    var _this7 = this;

    // IndexedDB에서 메시지 불러오기
    getMessagesFromRoom(roomName).then(function (messagesFromDB) {
      // 서버에서 메시지 불러오기
      socket.emit('join:room', roomName);
      socket.once('load:messages', function (messagesFromServer) {
        var allMessages = [].concat(_toConsumableArray(messagesFromDB), _toConsumableArray(messagesFromServer));
        _this7.setState({ currentRoom: roomName, messages: allMessages });
      });
    })['catch'](function (err) {
  ...
  handleCancelRoomSearch: function handleCancelRoomSearch() {
  ...
  renderRightPanel: function renderRightPanel() {
    if (this.state.currentRoom) {
      return React.createElement(
        ...
        React.createElement(MessageList, { messages: this.state.messages, user: this.state.username }),
        React.createElement(MessageForm, { onMessageSubmit: this.handleMessageSubmit, user: this.state.username, room: this.state.currentRoom })
      );
    } else {
      return React.createElement(
        'div',
  ...
  render: function render() {
    ...
    var loggedIn = _state2.loggedIn;
    var signup = _state2.signup;
    var roomSearched = _state2.roomSearched;
    var roomExists = _state2.roomExists;
    var rooms = _state2.rooms;
    var searchedRoom = _state2.searchedRoom;
    ...
    return React.createElement(
      ...
      loggedIn ? React.createElement(
        ...
        React.createElement(
          'div',
          { className: 'top-panel' },
          React.createElement('img', { src: '/images/INU.png', className: 'chatform-image', alt: 'Chatform Image' })
        ),
        React.createElement(
          'div',
          ...
          React.createElement(
            ...
            React.createElement(RoomList, {
              ...
              onRoomCreated: function (roomName) {
                return _this8.handleRoomCreated(roomName);
              },
              onCancel: function () {
                return _this8.handleCancelRoomSearch();
              },
              onRoomSelect: function (roomName) {
                return _this8.handleRoomSelect(roomName);
              },
              onRoomSearch: function (roomName) {
                return _this8.handleRoomSearch(roomName);
              }
            }),
            React.createElement(RoomSearchForm, { onRoomSearch: this.handleRoomSearch })
          ),
          React.createElement(
            'div',
            { className: 'right-panel' },
      ...
      ) : signup ? React.createElement(SignupForm, { onSignupSuccess: this.handleSignupSuccess, onBack: this.handleSignupBack }) : React.createElement(LoginForm, { onLogin: this.handleLogin, onSignupClick: this.handleSignupClick })
    );
  }
});
